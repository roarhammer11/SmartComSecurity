...
class WatchdogHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        if event.src_path.endswith(".db") and self.socket.connected_client != None:
            self.conn.execute("pragma busy_timeout=2147483647;")
            cursor = self.conn.execute(
                "SELECT hashId, fileData, fileId, fileName, modified FROM files ORDER BY timestamp DESC LIMIT 1;"
            )
            ...
            if (
                self.modifiedRow[4] != 0
            ):  # Checks if file was recently modified rather than created
